...
\title{Exact Discretization of Quantum Angular Momentum:\\
A Sparse-Matrix Construction Preserving SU(2) Commutation Relations}
...
\begin{abstract}
We present a discrete polar lattice construction that exactly preserves SU(2) angular momentum commutation relations on finite-dimensional matrices, enabling pedagogical visualization and computational quantum simulation. The lattice embeds quantum numbers $(n, \ell, m_\ell, m_s)$ as geometric coordinates on concentric rings, yielding sparse operators that satisfy $[L_i, L_j] = i\hbar\epsilon_{ijk}L_k$ to machine precision ($\sim 10^{-14}$) and produce $L^2$ eigenvalues $\ell(\ell+1)$ with 0.00\% relative error for all tested $\ell \leq 9$.
...
This lattice is a \textbf{pedagogical and computational tool}---not a physical model. We design the discretization to preserve SU(2) algebra exactly by embedding quantum numbers $(n, \ell, m_\ell, m_s)$ as geometric coordinates. Key construction choices include ring radii $r_\ell = 1 + 2\ell$, point density $N_\ell = 2(2\ell+1)$, and graph Laplacian operators respecting selection rules. These design decisions (inputs) yield exact commutators $[L_i, L_j] = i\hbar\epsilon_{ijk}L_k$ and eigenvalues $L^2 = \ell(\ell+1)$ (outputs) to machine precision.
...
\textbf{Construction and Validation (\S2--4):} We define a discrete polar lattice embedding quantum numbers as geometric coordinates and construct sparse-matrix operators via graph Laplacians. Validation confirms $[L_i, L_j] = i\hbar\epsilon_{ijk}L_k$ to $\sim 10^{-14}$ (machine precision) and $L^2$ eigenvalues $= \ell(\ell+1)$ with 0.00\% relative error for all $\ell \leq 9$. Complete spectral analysis verifies all 200 eigenvalues ($n=10$ system) match theory simultaneously---demonstrating \textbf{global algebraic exactness}.
...
This exactly reproduces hydrogen atom degeneracy.
...
Since spin operators use Pauli matrices (exact SU(2) representation), spin algebra is exact by construction:
\begin{equation}
[S_i, S_j] = i\hbar\epsilon_{ijk}S_k \quad \text{(machine precision)}
\end{equation}

\section{Validation: Exact Algebraic Structure}
...
\textbf{Assessment:} All deviations $< 10^{-14}$ (machine precision). The commutation relations are satisfied \textbf{exactly within numerical limits}.
...
\midrule
0 & 0.000 & 0.000000 & 0.0000\% & 2 & 2 \\
1 & 2.000 & 2.000000 & 0.0000\% & 6 & 6 \\
2 & 6.000 & 6.000000 & 0.0000\% & 10 & 10 \\
