...
\section{Topological Transition at $n=5$: Full 5D Decompactification}

The shell $n=5$ occupies a special role in hydrogen's spectrum: it is the first shell where all five orbital symmetries $(s, p, d, f, g)$ coexist. We interpret this as a \textit{topological phase transition} marking the decompactification of the full 5D bulk spacetime.

\subsection{Dimensional Hierarchy}

Consider the orbital angular momentum $l$ accessible at each shell:
\begin{align}
n=1: &\quad l=0 \quad (s \text{ only}) \quad \to \quad 1\text{D lattice} \\
n=2: &\quad l=0,1 \quad (s,p) \quad \to \quad 2\text{D lattice} \\
n=3: &\quad l=0,1,2 \quad (s,p,d) \quad \to \quad 3\text{D lattice} \\
n=4: &\quad l=0,1,2,3 \quad (s,p,d,f) \quad \to \quad 4\text{D lattice} \\
n=5: &\quad l=0,1,2,3,4 \quad (s,p,d,f,g) \quad \to \quad \mathbf{5\text{D lattice}}
\end{align}

At $n < 5$, the lattice is effectively lower-dimensional---only a subset of the full $SO(4,2)$ symmetry is realized. At $n=5$, the complete representation appears: a 4-simplex in quantum number space.

\subsection{Simplex Topology}

A $k$-simplex is the convex hull of $k+1$ vertices in $k$-dimensional space. For $n=5$:
\begin{itemize}
\item Vertices: 5 orbital types $(s, p, d, f, g)$
\item Edges: Transitions $\Delta l = \pm 1$ (electric dipole selection rules)
\item Faces: 2D subspaces spanned by three orbitals
\item Volume: 4D hypersurface embedded in 5D
\end{itemize}

This is a \textbf{4-simplex} (also called a \textit{pentatope} or 5-cell), the 4D analog of a tetrahedron. It has:
\begin{align}
\text{Vertices: } & 5 \\
\text{Edges: } & 10 \\
\text{Faces: } & 10 \text{ (triangles)} \\
\text{Cells: } & 5 \text{ (tetrahedra)} \\
\text{Euler characteristic: } & \chi = 5 - 10 + 10 - 5 = 0
\end{align}

The vanishing Euler characteristic $\chi = 0$ indicates a topological transition from compact to non-compact geometry.

\subsection{Decompactification Mechanism}

Below $n=5$, the lattice lives on a compact manifold (partial dimensional structure). At $n=5$, the full $SO(4,2)$ symmetry becomes active, and the AdS$_5$ bulk is fully decompactified.

In Kaluza-Klein theory, compact extra dimensions appear as towers of massive states. Below $n=5$, the "missing" orbital types correspond to heavy KK modes that are effectively decoupled. At $n=5$, all KK modes become accessible simultaneously---the compactification scale drops below the excitation energy.
...
The impedance $\kappa$ measures the holographic entropy ratio (Section VII). At $n < 5$, the bulk geometry is incomplete, so the entropy counting is off. At $n=5$, the full 5D bulk is realized, allowing correct holographic entropy matching. The fine structure constant $\alpha$ "locks" because \textit{this is where AdS$_5$ holography becomes fully operational}.
...
\item \textbf{Topological transition:} At $n=5$, all five orbital symmetries first coexist, marking the decompactification of the full 5D bulk. The fine structure constant "locks" at this threshold.
